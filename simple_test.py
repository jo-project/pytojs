...
"""
pytojs.eval_js(continue_labels_test)
...
"""
pytojs.eval_js(break_labels_test)

assert pytojs.eval_js('(new Date("2008-9-03T20:56:35.450686Z")).toString()')

assert pytojs.eval_js('/ser/.test("Mleko + ser to nabial")')
assert pytojs.eval_js('1 + "1"') == '11'

assert pytojs.eval_js('function (r) {return r}')(5) == 5

x, c = pytojs.run_file('examples/esprima.js')
...
assert pytojs.eval_js('var x = {get y() { return this.val;}, val: 11};x.y') == 11
try:
    assert pytojs.eval_js('syntax error!') and 0
except pytojs.PyJsException as err:
    assert str(err).startswith('SyntaxError: ')


assert pytojs.eval_js('pyimport time; time.time()') <= time.time()

pytojs.disable_pyimport()
try:
    assert pytojs.eval_js('pyimport time') and 0
except pytojs.PyJsException as err:
    assert str(err).startswith('SyntaxError: ')
...
pyobj = Foo(5)
context = pytojs.EvalJs({'foo': pyobj})
...
CryptoJS = pytojs.require('crypto-js@3.1.8')
data = [{'id': 1}, {'id': 2}]
...
AES = pytojs.require('crypto-js/aes@3.1.8')
...
esprima = pytojs.require('esprima')
escodegen = pytojs.require('escodegen')
...
#sample_js_code = pytojs.get_file_contents('examples/esprima.js')
...
# chalk = pytojs.require('chalk', include_polyfill=True)
